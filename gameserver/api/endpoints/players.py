...
async def create_player(
    player: PlayerCreate, current_user: User = Depends(get_current_active_user)
):
    """Create a new player."""
    ...
    new_player = Player(id=len(players_db) + 1, **player.model_dump(), active=True)
...
async def delete_player(
    player_id: int, current_user: User = Depends(get_current_active_user)
):
    """Delete a player."""
    for i, player in enumerate(players_db):
        if player.id == player_id:
...
async def join_game(
    player_id: int, game_id: int, current_user: User = Depends(get_current_active_user)
):
