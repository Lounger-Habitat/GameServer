...
log_file = os.path.join(LOGS_DIR, f"rotk_api_{datetime.now().strftime('%Y%m%d')}.log")
...
logger = logging.getLogger("rotk_api")
...
@router.post("/chat")
async def chat(param: ChatParam, current_user=Depends(get_current_active_user)):
    logger.info(f"Chat request received from user: {current_user.username}")
    logger.info(f"Using model: {param.model_id}")

    start_time = time.time()
    model = Model()
    try:
        res = model.chat(
            messages=[msg.model_dump() for msg in param.messages],
