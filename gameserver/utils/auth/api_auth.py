...
def get_user_by_api_key(api_key: str) -> Optional[TokenData]:
    ...
    try:
        payload = jwt.decode(
            api_key, SECRET_KEY, algorithms=[ALGORITHM], options={"verify_exp": False}
        )
...
async def _get_current_user(api_key: str = Depends(api_key_header)):
    """验证API Key并返回用户信息"""
    # 移除Bearer前缀(如果有)
...
async def get_current_active_user(current_user: TokenData = Depends(_get_current_user)):
    """获取当前活跃用户"""
    return current_user
